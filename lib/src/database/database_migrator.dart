...
abstract class BaseDatabaseMigrator {
  ...
  Future<void> _syncTables(Database database) async {
    ...
    for (BaseDatabaseModel model in _models) {
      if (!currentTables.contains(model.tableName))
        await _createTable(database, model);
      List<String> currentColumns =
          await _existingColumns(database, model.tableName);
      List<MapEntry<String, String>> newColumns =
          model.toSchema.entries.toList();
      List<String> queries = [];
      for (MapEntry<String, String> entry in newColumns) {
        bool columnExists = currentColumns.contains(entry.key);
        if (columnExists) continue;
        queries.add(
            "ALTER TABLE ${model.tableName} ADD COLUMN ${entry.key} ${entry.value};");
      }
  ...
  Future<void> _createTable<T extends BaseDatabaseModel>(
      Database db, T model) async {
  ...
  String _createTableQuery<T extends BaseDatabaseModel>(T model) {
    ...
    if (relations != null) {
      for (MapEntry<String, Map<String, String>> entry in relations.entries) {
        buffer.write(
            "FOREIGN KEY(${entry.key}) REFERENCES ${entry.value.keys.first}(${entry.value.values.first}), ");
      }
    }
    String tableValues =
        buffer.toString().replaceRange(buffer.length - 2, buffer.length, '');
