...
class BaseRepositoryGenerator {
  ...
  String template(String packageName) {
    ...
    buffer
        .write("import 'package:$packageName/services/database_manager.dart';");
    buffer.write("\n\n");
...
class RepositoryGenerator {
  ...
  String get _targetClassName =>
      "${_modelGenerator.targetClassName.replaceAll("Model", "")}Repository";
  ...
  String template(String packageName) {
    StringBuffer buffer = StringBuffer();
    buffer.write(
        "import 'package:$packageName/repositories/base_repository.dart';\n");
    buffer.write(
        "import 'package:$packageName/models/${_modelGenerator.fileName}.dart';\n\n");
    buffer.write("/// * ---------- Auto Generated Code ---------- * ///\n\n");
    buffer.write(
        "class ${_targetClassName} extends BaseRepository<${_modelGenerator.targetClassName}> {\n");
    buffer.write("\t");
    buffer.write(_generateConstructor);
    buffer.write("\n\n");
    buffer.write("}");
  ...
  String get _generateConstructor {
    ...
    buffer.write(
        "static final $_targetClassName _instance = $_targetClassName._();\n\t");
...
class DatabaseGenerator {
  ...
  String template(String packageName) {
    StringBuffer buffer = StringBuffer();
    buffer.write(
        "import 'package:flutter_pack/flutter_pack.dart' show BaseDatabaseManager;\n");
    buffer.write("import 'package:flutter_pack/src/model_manager.dart;\n");
    for (BaseModelGenerator generator in _generators) {
      if (generator.tableName == null) continue;
      buffer.write(
          "import 'package:$packageName/models/${generator.fileName}.dart';\n");
    }
    buffer.write("\n");
    buffer.write("class DatabaseManager extends BaseDatabaseManager {\n\t");
    buffer.write(
        "DatabaseManager._(): super(\"$packageName.db\", 1, _tables);\n\t");
    buffer.write(
    ...
    buffer.write("return [\n\t\t\t");
    String classes = _generators
        .where((element) => element.tableName != null)
        .map((element) => "${element.targetClassName}()")
        .join(",\n\t\t\t");
...
class DAOGenerator {
  ...
  String template(String packageName) {
    ...
    if (_generator.tableName != null)
      buffer.write(
          "static const String table = \"${_generator.tableName}\";\n\t");
    Map<String, Type> properties = _generator.properties;
    String fields = properties.entries
        .map((entry) =>
            "static const String ${entry.key} = \"${entry.key.toSnakeCase}\";")
        .join("\n\t");
    buffer.write(fields);
    buffer.write("\n");
    buffer.write("}");
...
class DAOClassGenerator {
  ...
  String template(String packageName) {
    ...
    for (BaseModelGenerator generator in _generators) {
      String repoName =
          "${generator.targetClassName.replaceAll("Model", "")}Repository";
      buffer.write(
          "import 'package:$packageName/repositories/${repoName.toSnakeCase}.dart';\n");
    }
    buffer.write("\n");
    ...
    for (BaseModelGenerator generator in _generators) {
      String repoName =
          "${generator.targetClassName.replaceAll("Model", "")}Repository";
      buffer.write(
          "static $repoName get ${repoName.toSnakeCase.toCamelCase} => ${repoName}.instance;\n\t");
    }
    buffer.write("\n}");
    return buffer.toString();
  }
