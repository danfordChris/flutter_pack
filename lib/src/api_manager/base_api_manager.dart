...
abstract class BaseAPIManager {
  ...
  Future<StarterAPIManagement>? get refreshOnUnauthorized;
  ...
  Future<APIResponse<T>> _apiResponse<T>(
    ...
    try {
      ...
      if (response.statusCode == 403) { 
        AppUtility.log("[Unauthorized] $url => Received 403 Forbidden. Attempting to refresh credentials...");
        try {
          StarterAPIManagement? shouldRetry = await refreshOnUnauthorized;
          if (shouldRetry != null) {
      ...
      // Ensure handling both Map and List response formats
      // if (responseBody is Map<String, dynamic>) {
      //   if (responseBody.containsKey('error') && responseBody['error'] != null) {
      //     final error = responseBody['message'] ?? responseBody['error'];
