...
abstract class BaseAPIManager {
  bool get isUsingDigitalSignature {
    return _starterAPIManagement?.privateKeyPEM != null &&
        _starterAPIManagement?.publicKeyPEM != null;
  }
  ...
  http.Client _createClient() {
    if (_starterAPIManagement?.allowedSHAFingerprints == null)
      return http.Client();
    return SecureHttpClient.build(
        _starterAPIManagement!.allowedSHAFingerprints!);
  }
  ...
  Future<http.Response> _response(
    ...
    AppUtility.log(
        "[*] ${type.label} => $_baseURL${_formattedParams(url, params)}");
    ...
    switch (type) {
      case _ResponseType._get:
        return await _client.get(_uri(_formattedParams(url, params)),
            headers: authHeaders);
      case _ResponseType._post:
        return await _client.post(_uri(url),
            headers: authHeaders, body: requestBody, encoding: encoding);
      case _ResponseType._put:
        return await _client.put(_uri(url),
            headers: authHeaders, body: requestBody, encoding: encoding);
      case _ResponseType._patch:
        return await _client.patch(_uri(url),
            headers: authHeaders, body: requestBody, encoding: encoding);
      case _ResponseType._delete:
        return await _client.delete(_uri(url),
            headers: authHeaders, body: requestBody, encoding: encoding);
    }
  }
  ...
  Future<void> _executeStatusCodeAction(int statusCode) async {
    ...
    if (action != null) {
      try {
        AppUtility.log(
            '[StatusCodeAction] Executing action for status code: $statusCode');
        await action();
      } catch (e) {
        AppUtility.log(
            '[StatusCodeAction] Error executing action for status code $statusCode: $e');
      }
    }
  }
  ...
  Future<APIResponse<T>> _apiResponse<T>(
    ...
    try {
      ...
      if (response.statusCode == 401 &&
          _starterAPIManagement?.refreshOnUnauthorized != null) {
        try {
          bool? shouldRetry =
              await _starterAPIManagement?.refreshOnUnauthorized;
          if (shouldRetry == true) {
          ...
          AppUtility.log(
              '[refreshOnUnauthorized] handler threw an exception: $e');
        }
      }
      ...
      if (contentType == null ||
        ...
        AppUtility.log(
            "[Format] $url => Unexpected content-type: $contentType");
        throw Exception(
            "The server did not return a JSON response (content-type: $contentType)");
      }
  ...
  Future<Map<String, String>?> _endpointHeaders(
    ...
    Map<String, String>? authHeaders =
        await _starterAPIManagement?.authorization;
